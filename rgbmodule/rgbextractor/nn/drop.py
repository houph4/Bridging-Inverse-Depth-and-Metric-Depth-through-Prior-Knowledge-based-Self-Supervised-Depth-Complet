...
# from fastervit.apps.trainer.run_config import Scheduler
from rgbmodule.rgbextractor.nn.ops import IdentityLayer, ResidualBlock
...
def apply_drop_func(network: nn.Module, drop_config: Optional[Dict[str, Any]]) -> None:
    ...
    if "name" not in drop_config:
        raise KeyError("Drop config must include 'name' key to specify the dropout function.")

    drop_func = drop_lookup_table.get(drop_config["name"])
    if not drop_func:
        raise ValueError(f"Drop function '{drop_config['name']}' is not implemented.")
