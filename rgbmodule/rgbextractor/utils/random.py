...
def torch_random_choices(
    ...
    if weight_list is None:
        rand_idx = torch.randint(low=0, high=len(src_list), generator=generator, size=(k,)).tolist()
        out_list = [src_list[i] for i in rand_idx]
    else:
        assert len(weight_list) == len(src_list), "The lengths of source list and weight list must match."
