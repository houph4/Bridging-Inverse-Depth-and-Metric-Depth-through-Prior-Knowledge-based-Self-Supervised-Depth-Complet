...
from datasets.void import VOID
...
def validate_model(model, val_loader, device='cuda'):
    ...
    with torch.no_grad():  # Disable gradient calculation
        ...
        for batch_idx, (rgb, lidar, metric_depth, real_depth) in enumerate(progress_bar):
            rgb = rgb.to(device)
...
def train_model_with_loader(model, optimizer, scheduler, train_loader, val_loader, epochs=40, device='cuda', start_epoch=0):
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch_idx, (rgb, lidar, metric_depth, depth) in progress_bar:
        ...
        # Optionally save the model at intermediate epochs
        if (epoch + 1) % 5 == 0:
            torch.save({
                'epoch': epoch,
            ...
            }, f'model_epoch_void_{epoch+1}.pt')
...
if __name__ == "__main__":
    ...
    val_dataset = VOID(mode='val')  # Validation mode
