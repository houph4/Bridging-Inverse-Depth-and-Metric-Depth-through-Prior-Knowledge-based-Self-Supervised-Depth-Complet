...
model = FreqFusion(hr_channels=64, lr_channels=32).to(device)
hr_feat = torch.rand(1, 64, 32, 32)
lr_feat = torch.rand(1, 32, 16, 16)
hr_feat = hr_feat.to(device)
lr_feat = lr_feat.to(device)
_, hr_feat, lr_feat = model(hr_feat=hr_feat, lr_feat=lr_feat) # lr_feat [1, 64, 32, 32]