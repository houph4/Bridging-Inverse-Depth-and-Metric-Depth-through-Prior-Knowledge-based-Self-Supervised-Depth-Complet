...
class NYU(BaseDataset):
    ...
    def __getitem__(self, idx):
        ...
        est = Image.fromarray(est_h5.astype('float32'), mode='F')
        ...
        if self.mode == 'train':
            ...
            est = TF.rotate(est, angle=degree)
            ...
            t_rgb = T.Compose([
                T.Resize((self.height, self.width)),  # 调整到 (256, 320)
                T.ToTensor(),
                T.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225))
            ])
        ...
        # est = TF.pad(est,padding=[8, 14], padding_mode='constant')
        # #est torch.Size([1, 228, 304])  est_a torch.Size([1, 256, 320])

        dep_sp *= self.mul_factor
        dep *= self.mul_factor
        est *= self.mul_factor
        return rgb, dep_sp, est, dep
